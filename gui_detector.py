...
class ArrowDetectorApp:
    def __init__(self, root):
        ...
        self.thread = None
        self.last_states = {}
        self.arrow_templates = self.load_templates()
    ...
    def setup_ui(self):
        ...
        for zone in self.zones:
            z_id = zone['id']
            lbl = tk.Label(zone_frame, text=f"Zone {z_id}: Waiting...", width=18, height=2, bg="gray", fg="white", font=("Arial", 14, "bold"))
            lbl.grid(row=0, column=z_id-1, padx=5, pady=5)
            ...
            while self.running:
                ...
                for zone in self.zones:
                    ...
                    for d, template in self.arrow_templates.items():
                        ...
                        if max_val > 0.5:
                            percent = int(max_val * 100)
                            self.log_to_gui(f"Zone {z_id} potential match: {d.upper()} - {percent}%")
                        if max_val > best_score:
                            best_score = max_val
                            best_dir = d
                    if best_score >= THRESHOLD:
                        states[z_id] = best_dir.upper()
                    else:
                        states[z_id] = None
